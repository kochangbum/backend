...
public class UserController {
	...
	public ResponseEntity<?> getUserInfo(HttpServletRequest request) {
		...
		// 데이터베이스에서 사용자 정보 조회
		try {
			User user = userService.findByUniqueNumber(request);  // userService에서 사용자 정보 조회
			if (user == null) {
	...
	public ResponseEntity<?> getFavoriteTeam(HttpServletRequest request) {
		// 데이터베이스에서 사용자 정보 조회
		try {
			User user = userService.findByUniqueNumber(request);
			if (user == null) {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("사용자를 찾을 수 없습니다.");
		}

			// 선호 구단 정보 반환
			return ResponseEntity.ok().body(Map.of("favoriteTeam", user.getUserFavoriteTeam()));
		} catch (Exception e) {
